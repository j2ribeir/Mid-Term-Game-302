...
function drawInstructions() {
  ...
  textSize(20);
  fill(255, 200, 100);
  text('ZONES', leftX, cy - 3);

  // Calm zone indicator
  textStyle(NORMAL);
  textSize(16);
  fill(255, 200, 100);
  circle(leftX + 8, cy + 22, 14);
  fill(190);
  text('Yellow glow = Calm Zone (reduces stress)', leftX + 22, cy + 22);

  // Scary zone indicator
  fill(200, 50, 50);
  circle(leftX + 8, cy + 47, 14);
  fill(190);
  text('Red glow = Danger Zone (increases stress)', leftX + 22, cy + 47);

  // Time boost indicator
  fill(0, 200, 255);
  circle(leftX + 8, cy + 72, 14);
  fill(190);
  text('Blue orb = Time Boost (+5 seconds)', leftX + 22, cy + 72);

  // --- Stress & Episodes ---
  textStyle(BOLD);
  textSize(20);
  fill(244, 67, 54);
  text('STRESS & EPISODES', leftX, cy + 110);

  textStyle(NORMAL);
  textSize(16);
  fill(190);
  text('Stress rises over time, faster when running', leftX, cy + 135);
  text('or in danger zones. At 100% stress, an episode', leftX, cy + 157);
  text('triggers: controls invert and vision shrinks.', leftX, cy + 179);

  // --- Timer ---
  textStyle(BOLD);
  textSize(20);
  fill(224);
  text('TIMER', leftX, cy + 217);

  textStyle(NORMAL);
  textSize(16);
  fill(190);
  text('You have 90 seconds. The timer is shown at the', leftX, cy + 242);
  text('top of the screen. If it hits 0, you lose.', leftX, cy + 264);
...
function drawHUD() {
  ...
  text('P = Pause | R = Restart | Shift = Run', padding, padding + 12);
}
